...
contract ERC20PoolFactoryTest is ERC20HelperContract {
    ...
    function testDeployERC20PoolMultipleTimes() external {
        address poolOne = _poolFactory.deployPool(
            address(_collateral),
            address(_quote),
            0.05 * 10 ** 18
        );
        ...
        address poolTwo = _poolFactory.deployPool(
            address(_collateral),
            compAddress,
            0.05 * 10 ** 18
        );
    ...
    function testDeployERC20PoolWithMinRate() external {
        _poolFactory.deployPool(
            address(new Token("Collateral", "C1")),
            address(new Token("Quote", "Q1")),
            0.01 * 10 ** 18
        );
    ...
    function testDeployERC20Pool() external {
        ...
        (uint256 interestRate, uint256 interestRateUpdate) = pool
            .interestRateInfo();
        assertEq(interestRate, 0.0543 * 10 ** 18);
        assertEq(interestRateUpdate, _startTime + 333);

        (uint256 poolInflator, uint256 lastInflatorUpdate) = pool
            .inflatorInfo();
    ...
    function testDeployERC20CompDaiPool() external {
        ...
        emit PoolCreated(poolAddress);
        ERC20Pool pool = ERC20Pool(
            _poolFactory.deployPool(compAddress, daiAddress, 0.0543 * 10 ** 18)
        );
    ...
    function testDeployERC20WbtcDaiPool() external {
        ...
        emit PoolCreated(poolAddress);
        ERC20Pool pool = ERC20Pool(
            _poolFactory.deployPool(wbtcAddress, daiAddress, 0.0543 * 10 ** 18)
        );
    ...
    function testDeployERC20WbtcUsdcPool() external {
        ...
        emit PoolCreated(poolAddress);
        ERC20Pool pool = ERC20Pool(
            _poolFactory.deployPool(wbtcAddress, usdcAddress, 0.0543 * 10 ** 18)
        );
    ...
    function testDeployERC20CompUsdcPool() external {
        ...
        emit PoolCreated(poolAddress);
        ERC20Pool pool = ERC20Pool(
            _poolFactory.deployPool(compAddress, usdcAddress, 0.0543 * 10 ** 18)
        );
    ...
    function testPoolAlreadyInitialized() external {
        vm.expectEmit(true, true, false, true);
        emit PoolCreated(poolAddress);
        address pool = _poolFactory.deployPool(
            address(_collateral),
            address(_quote),
            0.05 * 10 ** 18
        );
