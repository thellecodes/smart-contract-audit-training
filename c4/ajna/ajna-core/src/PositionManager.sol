    ...
    ReentrancyGuard
{
    ...
    function burn(
        ...
        if (positionIndexes[params_.tokenId].length() != 0)
    ...
    function memorializePositions(
        ...
        EnumerableSet.UintSet storage positionIndex = positionIndexes[
            params_.tokenId
        ];
        ...
        for (uint256 i = 0; i < indexesLength; ) {
            ...
            (uint256 lpBalance, uint256 depositTime) = pool.lenderInfo(
                index,
                owner
            );
    ...
    function mint(
        ...
        if (!_isAjnaPool(params_.pool, params_.poolSubsetHash))
            revert NotAjnaPool();
    ...
    function moveLiquidity(
        ...
        nonReentrant
    {
        Position storage fromPosition = positions[params_.tokenId][
            params_.fromIndex
        ];
        ...
        if (!positionIndex.remove(params_.fromIndex))
        ...
        (vars.lpbAmountFrom, vars.lpbAmountTo, ) = IPool(params_.pool)
            .moveQuoteToken(
                vars.maxQuote,
                params_.fromIndex,
                params_.toIndex,
        ...
        Position storage toPosition = positions[params_.tokenId][
            params_.toIndex
        ];
    ...
    function reedemPositions(
        ...
        for (uint256 i = 0; i < indexesLength; ) {
            ...
            if (position.depositTime == 0 || position.lps == 0)
                revert RemovePositionFailed();
    ...
    function _isAjnaPool(
        ...
        return (pool_ == erc20DeployedPoolAddress ||
            pool_ == erc721DeployedPoolAddress);
    }
        ...
        for (uint256 i = 0; i < indexesLength; ) {
            if (
                !_bucketBankruptAfterDeposit(
                    pool,
                    indexes[i],
                    positions[tokenId_][indexes[i]].depositTime
                )
            ) {
    ...
    function tokenURI(
        ...
        address collateralTokenAddress = IPool(poolKey[tokenId_])
            .collateralAddress();
        address quoteTokenAddress = IPool(poolKey[tokenId_])
            .quoteTokenAddress();

        PositionNFTSVG.ConstructTokenURIParams memory params = PositionNFTSVG
