...
import {IERC20Pool, IERC20PoolBorrowerActions, IERC20PoolImmutables, IERC20PoolLenderActions} from "./interfaces/pool/erc20/IERC20Pool.sol";
import {IERC20Taker} from "./interfaces/pool/erc20/IERC20Taker.sol";

import {IPoolLenderActions, IPoolKickerActions, IPoolTakerActions, IPoolSettlerActions} from "./interfaces/pool/IPool.sol";
import {IERC3156FlashBorrower, IERC3156FlashLender} from "./interfaces/pool/IERC3156FlashLender.sol";

import {DrawDebtResult, RepayDebtResult, SettleParams, SettleResult, TakeResult} from "./interfaces/pool/commons/IPoolInternals.sol";
import {PoolState} from "./interfaces/pool/commons/IPoolState.sol";

import {FlashloanablePool} from "./base/FlashloanablePool.sol";

import {_getCollateralDustPricePrecisionAdjustment, _roundToScale, _roundUpToScale} from "./libraries/helpers/PoolHelper.sol";
import {_revertIfAuctionClearable, _revertOnExpiry} from "./libraries/helpers/RevertsHelper.sol";
...
contract ERC20Pool is FlashloanablePool, IERC20Pool {
    ...
    function drawDebt(
        ...
        collateralToPledge_ = _roundToScale(
            collateralToPledge_,
    ...
    function repayDebt(
        ...
        collateralAmountToPull_ = _roundToScale(
    ...
    function addCollateral(
        ...
        if (amountToAdd_ != 0 && amountToAdd_ < _bucketCollateralDust(index_))
            revert DustAmountNotExceeded();
        amountToAdd_ = _roundToScale(
            amountToAdd_,
        ...
        _updateInterestState(
            poolState,
            Deposits.getLup(deposits, poolState.debt)
        );
    ...
    function take(
        ...
        result.quoteTokenAmount = _roundUpToScale(
    ...
    function _isFlashloanSupported(
        ...
        return
            token_ == _getArgAddress(QUOTE_ADDRESS) ||
            token_ == _getArgAddress(COLLATERAL_ADDRESS);
    }
    ...
    function _bucketCollateralDust(
        ...
        uint256 pricePrecisionAdjustment = _getCollateralDustPricePrecisionAdjustment(
                bucketIndex_
            );
